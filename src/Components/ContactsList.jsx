...
import { set,add,addFav,removeFav} from '../Store/Actions';
...
export default function ContactsList(props){
   ...
   const title=props.favContacts?"Favourties...":"Contacts"
    ...
    const renderList=(props.searchTerm?(contactsList.filter(i=>Object.values(i).join('').toLowerCase().includes(props.searchTerm)))  :  (t) ).map((item,index)=>{
        ...
        <div className="contactListCard" key={index}>
    ...
    <span onClick={()=>handleFav(item)} style={{position:'absolute',right:'6em',top:'0em',cursor:'pointer'}}>{
        ...
        :<svg xmlns="http://www.w3.org/2000/svg" height="16" width="16" viewBox="0 0 512 512">
            <path fill='red' d="M47.6 300.4L228.3 469.1c7.5 7 17.4 10.9 27.7 10.9s20.2-3.9 27.7-10.9L464.4 300.4c30.4-28.3 47.6-68 47.6-109.5v-5.8c0-69.9-50.5-129.5-119.4-141C347 36.5 300.6 51.4 268 84L256 96 244 84c-32.6-32.6-79-47.5-124.6-39.9C50.5 55.6 0 115.2 0 185.1v5.8c0 41.5 17.2 81.2 47.6 109.5z"/></svg>
    }</span>
    ...
    <span style={{display:'flex',justifyContent:'center'}}>{title}</span>
        <div className="list-main" style={{display:'flex',flexWrap:'wrap'}}>
            
          
            {renderList?.length>0?<>{renderList}</>:<div style={{ width:'100%',
                backgroundColor:'#0080a7e4',
    borderRadius:'5em',display:'flex',justifyContent:'center',alignItems:'center',position:'relative',top:'10em',}}>No {title} Found...</div>
            }
